...
export const Route = createFileRoute("/simulation/new")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Configure pipeline operating conditions and run a flow assurance prediction." },
    ],
  }),
      ...
      <form onSubmit={onSubmit} className="mx-auto flex max-w-[1400px] flex-col gap-6">
        ...
        >
          <div>
            ...
            <h1 className="mt-1 text-2xl font-semibold tracking-tight sm:text-3xl">New Simulation</h1>
            <p className="mt-1 max-w-2xl text-sm text-muted-foreground">
              Define operating conditions, environment, fluid properties and mitigation strategy. The
              FAIP engine runs transient thermal simulation, feature engineering and ML-based risk
              assessment.
            </p>
          </div>
        ...
        <FormSection index="A" title="Operating Conditions" description="Pipeline inlet conditions and produced fluid flow rates.">
        ...
        <FormSection index="B" title="Environmental Conditions" description="Subsea environmental envelope around the pipeline.">
        ...
        <FormSection index="C" title="Fluid Properties" description="Compositional and PVT characteristics of the production stream.">
          ...
          <FieldInput<PredictionInput> name="asphaltene_index" label="Asphaltene Index" unit="idx" />
        </FormSection>

        <FormSection index="D" title="Asset & Mitigation" description="Pipeline asset metadata and active mitigation strategy.">
          <FieldInput<PredictionInput> name="insulation" label="Insulation Efficiency" unit="%" />
          <FieldInput<PredictionInput> name="chemical_injection" label="Chemical Injection Rate" unit="L/h" />
        ...
        <div className="sticky bottom-4 z-10 flex items-center justify-end gap-2 rounded-xl border border-border/60 bg-card/80 p-3 backdrop-blur-xl">
          <span className="mr-auto text-xs text-muted-foreground">
            {predict.isPending ? "Running transient thermal simulation + ML inference…" : "Ready to run prediction."}
          </span>
          <Button
            type="button"
            variant="ghost"
