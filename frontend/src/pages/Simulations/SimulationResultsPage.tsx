...
import { ThermalChart } from "@/components/charts/thermal-chart";
import { RiskRadar } from "@/components/charts/risk-radar";
import { opsStatusMeta, riskBgClass, riskTextClass } from "@/lib/risk";

export const Route = createFileRoute("/simulation/results")({
  head: () => ({
    meta: [
      { title: "Simulation Results — FAIP" },
      { name: "description", content: "Predictive flow assurance risk dashboard, thermal profile and engineering recommendations." },
    ],
  }),
    ...
    <div className="mx-auto flex max-w-[1600px] flex-col gap-6">
      ...
      >
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            Generated {new Date(result.created_at).toLocaleString()} · Transient thermal + ML risk inference
          </p>
        </div>
        ...
        >
          ...
          <CardContent>
            <div className="flex flex-wrap items-end justify-between gap-6">
              <div>
                <div className={`font-mono text-6xl font-bold ${riskTextClass(result.overall_risk.level)}`}>
              ...
              <p className="max-w-md text-sm text-muted-foreground">{result.overall_risk.summary}</p>
            </div>
          </CardContent>
        </Card>
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        <Card className="border-border/60 bg-card/80 lg:col-span-2">
          <CardHeader className="pb-2">
            <SectionHeading n="03" title="Transient Thermal Analysis" subtitle="Pipeline temperature evolution during shutdown." inline />
          </CardHeader>
          <CardContent>
            <ThermalChart series={result.transient} hydrateEqT={result.physics.hydrate_equilibrium_temperature} />
            <div className="mt-3 grid grid-cols-2 gap-3">
              <KpiPill label="Initial Temperature" value={`${result.transient.initial_temperature.toFixed(2)} °C`} icon={Thermometer} />
              <KpiPill label="Minimum Temperature" value={`${result.transient.minimum_temperature.toFixed(2)} °C`} icon={Snowflake} />
            </div>
          </CardContent>
        </Card>
        ...
        <Card className="border-border/60 bg-card/80">
          <CardHeader className="pb-2">
            <SectionHeading n="09" title="Risk Visualization" subtitle="Multi-mode comparison." inline />
          </CardHeader>
          <CardContent>
      ...
      <div>
        <SectionHeading n="04 / 05" title="Hydrate & Physics Analysis" subtitle="Subcooling, equilibrium and physical interpretation." />
        <div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          ...
          <KpiCard
            ...
            hint={result.physics.hydrate_risk_flag ? "Operating inside hydrate envelope" : "Outside hydrate envelope"}
          />
          <KpiCard
        ...
        <Card className="border-[color:var(--risk-medium)]/40 bg-[color:var(--risk-medium)]/10">
          ...
          <CardContent className="space-y-2">
            {result.insights.map((m, i) => (
              <div key={i} className="flex items-start gap-2 rounded-md border border-[color:var(--risk-medium)]/30 bg-background/30 p-3 text-sm">
        ...
        <Card className="border-primary/30 bg-primary/5">
          ...
          <CardContent className="space-y-2">
            {result.recommendations.map((r, i) => (
              ...
              >
                <Badge variant="outline" className="mt-0.5 border-primary/40 bg-primary/10 font-mono text-[10px] text-primary">
...
export { ResultsPage as SimulationResultsPage };
  ...
  return (
    ...
    >
      <CardContent className="p-4">
        ...
        <div className={`mt-2 font-mono text-xl font-semibold ${danger ? "text-[color:var(--risk-high)]" : "text-foreground"}`}>
          {value}
        </div>
