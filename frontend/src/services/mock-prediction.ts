...
export function mockPredict(input: PredictionInput): PredictionResult {
  ...
  const hydrateP = clamp(
    cooling * 0.7 +
      (1 - input.insulation / 100) * 0.3 -
      input.chemical_injection / 200,
    0.02,
    0.99,
  );
  const waxP = clamp(
    input.wax_content / 30 + (1 - input.insulation / 100) * 0.25,
    0.02,
    0.99,
  );
  ...
  for (let i = 0; i < N; i++) {
    ...
    const temp =
      input.T_seawater + (input.T_inlet - input.T_seawater) * Math.exp(-t / tau);
  ...
  if (waxP > 0.5)
    insights.push("Elevated wax deposition risk — wall temperature approaches WAT.");
  if (corrP > 0.5)
  ...
  if (input.insulation < 70) recommendations.push("Improve pipeline insulation or add active heating.");
  if (overallLevel === "HIGH") recommendations.push("Initiate controlled restart procedure with operator on standby.");
  ...
  if (recommendations.length === 0) recommendations.push("Maintain current operating envelope; continue routine monitoring.");
